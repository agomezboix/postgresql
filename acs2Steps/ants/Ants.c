...
void setNextStep(Ants *ants, int sizeProblem, int alpha, int beta, double q0)
{
    double sumAj=0;
    double q, maxValue, indexMax;
    double arrayTemp[sizeProblem];
    int position=ants->cantState-1;
    int currentState=ants->tourMemory[position];
    int i;
    double temp=0;
    double aux=0;
    double pheromone;
    ...
    for(i=0; i<sizeProblem; i++)
    {
        if(!visited(*ants,i))
        {
            ...
            temp=pheromone * exponential(aux, beta);
            sumAj+=temp;
        }
        arrayTemp[i]=temp;
    ...
    if(q<=q0)
    {
    	maxValue=arrayTemp[0];
    	indexMax=0;
    	for(i=1;i<sizeProblem;i++)
    	{
    		if(maxValue<arrayTemp[i])
    		{
    			maxValue=arrayTemp[i];
    			indexMax=i;
    		}
    	}
    	//se le suma 1 a indexMax porque los valores de los estados son numeros desde 1 a sizeProblem incluyendo ambos n�meros
    	setState(ants,indexMax+1);
    }
    else
    {
    	for (i = 0; i < sizeProblem; i++)
