...
RelOptInfo * executeACS2Pasos(PlannerInfo *root, int number_of_rels, List *initial_rels)
{
	...
	for (i = 1; i <= cantCicle; i++)
	{
		...
		countState=1;
		while(countState < number_of_rels)
		{
		...
		bestLocalSolution=searchBestSolution(root,arrayAnts,cantAnts,number_of_rels,0);//modificar

		fprintf(f1, "%d  \n", i);

		for (w = 0; w < cantAnts; w++) {
	...
	fprintf(f, "acs  %d, %If, t: %If,c: %d, a:%d", number_of_rels,
			globalSolution.cost,
