...
RelOptInfo * executeACS2Pasos(PlannerInfo *root, int number_of_rels, List *initial_rels)
{
	...
	int * tmp,w,x;
	double alpha,beta,pheromoneInicial,constEvaporation,**matrix;
	double r=0.3;
	alpha=5;
	...
	inicializeMatrixPheromone(number_of_rels,constEvaporation);
	...
	for (i = 1; i <= cantCicleStepOne; i++)
	{
		...
		while(countState < sizeProblemStepOne)
		{
			...
			{
				setNextStep(root,&arrayAnts[k],number_of_rels,alpha,beta);
			}
			countState++;
        ...
        calculate_PheromoneMaxMin(bestLocalSolution.cost,number_of_rels,constEvaporation);
    ...
    for (i = 1; i <= cantCicleStepTwo; i++)
    {
		...
		updatePheromone_Global(globalSolution, number_of_rels, constEvaporation,
				sizeProblemStepTwo);
		calculate_PheromoneMaxMin(bestLocalSolution.cost, number_of_rels,
				constEvaporation);
		regulate_PheromoneMaxMin(number_of_rels);
	}
	...
	fprintf(f, "mm2S  %d, %If, t: %If,c: %d, a:%d", number_of_rels,
			globalSolution.cost,
